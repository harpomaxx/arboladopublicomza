...
1. Se puede considerar alguna sección mas peligrosa que otra?
2. Se puede considerar alguna especie mas peligrosa que otra?
3. ** Se puede predecir el grado de peligrosidad de una arbol dado sus caracteristicas?**
...
5. Creamos la clase **inclinacion_grave** con valores si no
```{r, eval=FALSE, include=FALSE}
data_arbolado<-data_arbolado %>% mutate(inclinacion_grave=ifelse(inclinacion=="Grave (> 30ø)","si","no"))
data_arbolado$inclinacion_grave<-as.factor(data_arbolado$inclinacion_grave)
data_arbolado %>% group_by(inclinacion_grave) %>% summarise(n=n())
...
data_arbolado
data_arbolado %>% group_by(especie) %>% summarise(total=n(),incl_sum=sum(ifelse(inclinacion_grave=='si',1,0)),percent=incl_sum/total)%>% arrange(desc(percent))
```
...
ggmap(mza_map)+ 
  geom_point(aes(x= long, y=lat,color=especie),data=data_arbolado %>% filter(inclinacion_grave=='si'),size=0.4)
```


```{r, fig.height=6}
histogram(~especie,data=data_arbolado,scales = list(x=list(rot=45)))
histogram(~especie|inclinacion_grave,data=data_arbolado,scales = list(x=list(rot=45),cex=0.6))
```
...
```{r, eval=FALSE, fig.height=8, message=FALSE, warning=FALSE, include=FALSE}
mza_map <- get_map(location = "mendoza", zoom = 14,maptype = "roadmap",color = "bw")
data_inclinacion=data_arbolado  %>% filter(inclinacion_grave=="si")
...
```{r fig.height=8}
inclinacion_por_seccion=data_arbolado %>% group_by(seccion,inclinacion_grave) %>% summarise(n=n()) %>%  mutate(freq=(n/sum(n))*100)
barchart(n~factor(seccion),data=inclinacion_por_seccion,groups=inclinacion_grave,auto.key=T )
...
```{r, eval=FALSE, include=FALSE}
library(d3heatmap)
cor_matrix<-cor(data_arbolado %>% select(-especie,-seccion,-inclinacion_grave))
d3heatmap(cor_matrix,colors = "Reds")
```

### Clustering 
```{r clustering, fig.height=8}
#data_arbolado <- downSample(x = data_arbolado,  y = data_arbolado$inclinacion_grave, yname="class") %>% select(-class)
data_arbolado %>% group_by(inclinacion_grave) %>% summarise(n=n())
...
data_arbolado_cluster %>% group_by(cluster) %>% summarise(inclinados=sum(inclinacion_grave=="si"),total=n(),percent=inclinados/total) %>% arrange(desc(percent))
# Promedios
...
pca_arbolado<-data.frame(pca$x,
                         inclinacion_grave=data_arbolado$inclinacion_grave,
...
g<-ggplot(sample_pca_arbolado,aes(x=PC1,y=PC2))+
  geom_jitter(aes(color=as.factor(inclinacion_grave),text=cluster,shape=inclinacion_grave    ))+
...
#data_arbolado_cluster %>% group_by(cluster) %>% summarise(sum_inc=sum(inclinacion_grave=="si"))
...
trainIndex <- createDataPartition(trainset$inclinacion_grave, p=0.70, list=FALSE)
...
#train_formula<-formula(inclinacion_grave~altura+diametro_tronco_cm+lat+long+area_seccion+especie)
#train_formula<-formula(inclinacion_grave~PC1+PC2+PC5+cluster)
...
```{r}
#data_train_upsam <- downSample(x = data_train,  y = data_train$inclinacion_grave, yname="class")
#data_train_upsam <-SMOTE(inclinacion_grave~.,data=data_train,perc.over = 200,perc.under=200)
#data_train_upsam<-data_train[sample(nrow(data_train),nrow(data_train)/10),]
data_train_upsam<-data_train
data_train_upsam %>% group_by(inclinacion_grave) %>% summarise(n=n())
...
model_weights <- ifelse(data_train_upsam$inclinacion_grave == "si",
                        (1/table(data_train_upsam$inclinacion_grave)[2]) * 0.5,
                        (1/table(data_train_upsam$inclinacion_grave)[1]) * 0.5)
svmGrid <-  expand.grid(C= c(0.5,0.4,0.3,0.2,0.1), 
...
train_formula<-formula(inclinacion_grave~altura+diametro_tronco_cm+lat+long)
...
confusionMatrix(predsnnetsamp,data_test$inclinacion_grave)
```

### Linear Support Vector Machines
```{r}
train_formula<-formula(inclinacion_grave~altura+diametro_tronco_cm+lat+long+area_seccion)
...
confusionMatrix(predssvml_samp,data_test$inclinacion_grave)
```
...
svmr_model<- train(train_formula,
               data = data_train_upsam,
               tuneLength = 8,
               method = "svmRadial",   # linear kernel
               preProcess=c("scale","center"),
               metric="ROC",
               trControl = ctrl_fast)
...
```{r}
train_formula<-formula(inclinacion_grave~altura+diametro_tronco_cm+lat+long+area_seccion+especie)
tree_model<- train(train_formula,
               data = data_train_upsam,
               method = "rpart",
               tuneLength = 8,
               preProcess=c("scale","center"),
               metric="ROC",
               trControl = ctrl_fast)
preds_tree_probsamp=predict(tree_model,data_test,type='prob')
preds_tree_samp=ifelse(preds_tree_probsamp$si >=0.5,'si','no')
confusionMatrix(preds_tree_samp,data_test$inclinacion_grave)
```
...
confusionMatrix(predsrfsamp,data_test$inclinacion_grave)
```
...
predsrfsamp=ifelse(predsrfprobsamp$si >=0.58,'si','no')
confusionMatrix(predsrfsamp,data_test$inclinacion_grave)
```
...
```{r}
#train_formula<-formula(inclinacion_grave~altura+diametro_tronco_cm+lat+long+area_seccion+especie)
train_formula<-formula(inclinacion_grave~altura+diametro_tronco_cm+lat+long+area_seccion+cluster+especie)
...
confusionMatrix(predsrf_cluster_samp,data_test$inclinacion_grave)
```
